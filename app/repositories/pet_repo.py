...
async def update_my_pet(id: str, pet: UpdatePetSchema, db: Session, user_id: str):
    ...
    if not updated_pet:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f'Pet not found')
    if updated_pet.owner_id != uuid.UUID(user_id):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail='You are not allowed to perform this action')
    pet_query.update(pet.dict(exclude_unset=True), synchronize_session=False)
    db.commit()
    return updated_pet

# update pet without authentication
async def get_pet(id: str, db: Session):
    pet = db.query(models.Pet).filter(models.Pet.unique_id == id).first()
    if not pet:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f"No pets found")
    return pet

# get pets with authentication
async def get_my_pet(id: str,db: Session, user_id: str):
